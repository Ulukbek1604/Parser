...
# async def problem_2(message: types.Message)
...
#     await bot_dp.sql_command_random(message)
async def show_random_menu(message: types.Message):
    await bot_dp.random_menu(message)
...
def register_hendlers_client(dp: Dispatcher):
    ...
    dp.register_message_handler(show_random_menu, commands=['random'])
